...
int servopin = 12;    //定义舵机信号引脚接数字12脚

bool boolen=false;
//写上你连接的无线WiFi账号密码  

const char* ssid     = "Blast";
const char* password = "435684593";

//定义开门引脚
#define relay_pin 2 //焊接或跳线到继电器开门
...
void open_door(WebsocketsClient &client) {
  if (digitalRead(relay_pin) == LOW) {
    digitalWrite(relay_pin, HIGH); //关闭（通电），使门解锁
    Serial.println("Door Unlocked");
    client.send("door_open");
    door_opened_millis = millis(); // 门打开
  }
}
